...
| --- | --- | --- |
| [BEAST-01 rebuild — execution ladder](2026-08-27-beast-rebuild-execution-ladder.md) | Companion to the clean-room rebuild plan held on [PR #247](https://github.com/MooseGooseConsulting/RobotOverview/pull/247): rung-by-rung execution order (ratify → extract + memory triage → pin the world → CI proof → `beast_hardware` → C1 side-by-side → C2 fresh metal → cutover + delete → close out), the researched deploy-artifact decision (container vs release-dir A/B), and the four owner decisions that gate rung 1. The robot is never broken below rung 7. | Rung 0 (owner ratification) gates all execution |
